...
from .token import AnaType, Token, TokenType
...
class QValue:
    ...
    def to_dict(self, state_len: int, action_len: int, field_permutation: bool, config: DataConfig):
        ...
        values[np.logical_not(self.valid_mask)] = -1
...
def _teacher_config(config: Optional[DataConfig], name: str, default: float):
    return getattr(config, name, default) if config is not None else default


def update_teacher_determine_action_values(action_space: Sequence, positive_actions: Optional[Set[Token]],
                                           config: Optional[DataConfig] = None):
    exact_reward = _teacher_config(config, "update_teacher_exact", 0.95)
    default_reward = _teacher_config(config, "update_teacher_default", 0.05)
    same_token_reward = _teacher_config(config, "update_teacher_same_token", 0.10)
    field_reward = _teacher_config(config, "update_teacher_field", 0.15)
    same_field_type_reward = _teacher_config(config, "update_teacher_same_field_type", 0.35)

    if not positive_actions:
        return [default_reward] * len(action_space)

    positive_types = {action.type for action in positive_actions}
    positive_fields = [action for action in positive_actions if action.type is TokenType.FIELD]
    rewards = []
    for action in action_space:
        if action in positive_actions:
            rewards.append(exact_reward)
        elif action.type is TokenType.FIELD and positive_fields:
            same_field_type = any(action.field_type == positive.field_type for positive in positive_fields)
            rewards.append(same_field_type_reward if same_field_type else field_reward)
        elif action.type in positive_types:
            rewards.append(same_token_reward)
        else:
            rewards.append(default_reward)
    return rewards


determine_action_values = update_teacher_determine_action_values
...
class TableQValues(Dataset):
    ...
    def __getitem__(self, index) -> QValue:
        ...
        action_values = determine_action_values(self.table.action_space, positive_actions, self.table.config)
