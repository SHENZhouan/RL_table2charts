...
def _reward_config(config: Optional[DataConfig], name: str, default: float):
    return getattr(config, name, default) if config is not None else default


def update_reward_determine_action_values(action_space: Sequence, positive_actions: Optional[Set[Token]],
                                          config: Optional[DataConfig] = None):
    exact_reward = _reward_config(config, "update_reward_exact", 0.95)
    default_reward = _reward_config(config, "update_reward_default", 0.05)
    same_token_reward = _reward_config(config, "update_reward_same_token", 0.10)
    field_reward = _reward_config(config, "update_reward_field", 0.15)
    same_field_type_reward = _reward_config(config, "update_reward_same_field_type", 0.35)
...
determine_action_values = update_reward_determine_action_values
