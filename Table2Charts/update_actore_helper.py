...
from model.nn.update_actore_copyNet import UpdateActoreCopyNet
from util import log_params, load_optimizer, set_no_calculate_gradient, load_extra, save_checkpoint, load_states


def load_update_actore_checkpoint(file_path: str, module: Module, device="cpu"):
    states = load_states(file_path)
    incompatible = module.load_state_dict(states["module_state"], strict=False)
    missing = getattr(incompatible, "missing_keys", [])
    unexpected = getattr(incompatible, "unexpected_keys", [])
    logger = logging.getLogger(f"load_update_actore_checkpoint({dist.get_rank()})")
    if missing:
        logger.info("Missing keys while loading checkpoint into update_actore model: %s", missing)
    if unexpected:
        logger.info("Unexpected keys while loading checkpoint into update_actore model: %s", unexpected)
    module.to(device)
...
def create_model(args):
    ...
    if args.model_name == "cp":
        ...
        model = UpdateActoreCopyNet(model_config)
        if dist.get_rank() == 0:
            logger.info("UpdateActoreCopyNetConfig: {}".format(vars(model_config)))
    else:
        raise NotImplementedError(f"{args.model_name} not yet implemented.")

    experiment_name = datetime.now(timezone(timedelta(hours=8))).strftime("%Y%m%d%H%M%S-update_actore-") + \
...
def prepare_model(model: Module, device, args):
    ...
    if args.pre_model_file:
        ...
        load_update_actore_checkpoint(args.pre_model_file, model, device)
    else:
    ...
    if args.fresh_decoder:
        if isinstance(model, UpdateActoreCopyNet):
