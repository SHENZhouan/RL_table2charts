...
from update_actore_helper import construct_data_config, create_model, prepare_model
from model import DEFAULT_MODEL_SIZES, DEFAULT_MODEL_NAMES
from os import path, getpid
from reinforce.student.update_actore_config import StudentConfig
from reinforce.student.update_actore_student import UpdateActoreStudent
...
def parse_args():
    ...
    parser.add_argument("--actor_loss_weight", default=0.1, type=float,
                        help="Weight for the update_actore policy-gradient style actor loss.")
    ...
    parser.add_argument("--critic_score_weight", default=0.5, type=float,
                        help="Blend weight for critic scores during training search. 1.0 uses critic only; "
                             "0.0 uses actor policy only.")
    parser.add_argument("--use_actor_in_eval", action="store_true",
                        help="Use blended actor/critic scores during validation instead of critic-only scores.")
...
def iteration(epoch: int, student: UpdateActoreStudent, is_testing: bool, n_tables: int, args, local_tuids=None):
...
def main(args):
    ...
    experiment_name += '-actor-critic-RL'
    ...
    student_config = StudentConfig(optimizer, criterion,
                                   ...
                                   use_actor_in_eval=args.use_actor_in_eval)
    ...
    student = UpdateActoreStudent(student_config, data_config, search_config, ddp, args.apex,
