...
from reinforce.student.update_reward_policy_student import Student
...
def parse_args():
    ...
    parser.add_argument('--update_reward_exact', default=0.95, type=float,
                        help='dense reward for an exact ground-truth next action')
    parser.add_argument('--update_reward_default', default=0.05, type=float,
                        help='dense reward for ordinary non-target actions')
    parser.add_argument('--update_reward_same_token', default=0.10, type=float,
                        help='dense reward for non-exact actions with the same token type as a target action')
    parser.add_argument('--update_reward_field', default=0.15, type=float,
                        help='dense reward for non-exact field actions when the target action is a field')
    parser.add_argument('--update_reward_same_field_type', default=0.35, type=float,
                        help='dense reward for non-exact field actions matching a target field type')
    parser.add_argument('--update_reward_positive_threshold', default=0.5, type=float,
                        help='threshold used only for binary training metrics from dense rewards')
    parser.add_argument('--policy_epsilon_start', default=0.2, type=float,
                        help='initial epsilon for updated-policy training exploration')
    parser.add_argument('--policy_epsilon_end', default=0.02, type=float,
                        help='minimum epsilon for updated-policy training exploration')
    parser.add_argument('--policy_epsilon_decay', default=0.8, type=float,
                        help='epoch-level epsilon decay for updated-policy training exploration')
    parser.add_argument('--policy_explore_top_m', default=5, type=int,
                        help='sample among the top-M scored valid actions when epsilon exploration is selected; '
                             'set <= 0 to sample among all valid actions')
...
def main(args):
    ...
    data_config.update_reward_exact = args.update_reward_exact
    data_config.update_reward_default = args.update_reward_default
    data_config.update_reward_same_token = args.update_reward_same_token
    data_config.update_reward_field = args.update_reward_field
    data_config.update_reward_same_field_type = args.update_reward_same_field_type
    if args.local_rank == 0:
    ...
    student_config.update_reward_negative_weight = args.negative_weight
    student_config.update_reward_positive_threshold = args.update_reward_positive_threshold
    student_config.policy_epsilon_start = args.policy_epsilon_start
    student_config.policy_epsilon_end = args.policy_epsilon_end
    student_config.policy_epsilon_decay = args.policy_epsilon_decay
    student_config.policy_explore_top_m = args.policy_explore_top_m
