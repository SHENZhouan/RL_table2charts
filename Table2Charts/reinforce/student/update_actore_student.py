...
from update_actore_helper import save_ddp_checkpoint
...
from search.agent.update_actore_drill_down import UpdateActoreBeamDrillDownAgent
from util import to_device, scores_from_confusion, time_str
from .update_actore_config import StudentConfig
...
class UpdateActoreStudent:
    ...
    def add_table(self, tUID: str):
        self.agents.add(UpdateActoreBeamDrillDownAgent(tUID, self.data_config, self.special_tokens,
                                                       self.search_config))
    ...
    def _rank_scores_(self, output: dict, valid_mask: torch.Tensor, force_critic: bool = False):
        ...
        critic_weight = self.config.critic_score_weight
        return critic_weight * critic_scores + (1.0 - critic_weight) * actor_probs
    ...
    def _feed_batch_nn_(self, mode: BatchMode, samples: List[QValue], noise_fn: Callable = None) -> np.ndarray:
        ...
        with torch.set_grad_enabled(mode is BatchMode.Train):
            if mode is BatchMode.Estimate:  # Return action value estimations
                ...
                estimates = self._rank_scores_(output, valid_mask).detach().cpu().numpy()
                ...
                if self.local_rank == 0:
                    ...
                    self.summary_writer.add_scalar("{0}/critic_loss".format(phase_tag), critic_loss.item(),
                                                   global_step)
                    self.summary_writer.add_scalar("{0}/actor_loss".format(phase_tag), actor_loss.item(),
                                                   global_step)
                ...
                if mode is BatchMode.Test:
                    use_critic = not self.config.use_actor_in_eval
                    valid_mask = target != -1
                    return self._rank_scores_(output, valid_mask, force_critic=use_critic).detach().cpu().numpy()
    ...
    def dist_summary(self):  # Sync point!
        ...
        recall_tensor = torch.tensor([
            complete_recall['@01'] * final_stage_cnt,
            complete_recall['@03'] * final_stage_cnt,
            complete_recall['@05'] * final_stage_cnt,
            complete_recall['@10'] * final_stage_cnt,
        ], device=self.device, dtype=torch.double)
        ...
        if self.local_rank == 0:
            ...
            total_success = cnt_tensor[1].item()
            total_final_stage = cnt_tensor[2].item()
            info_tensor = info_tensor.cpu().numpy()
            state_info = info_tensor[:5] / total_success if total_success > 0 else info_tensor[:5]
            time_info = info_tensor[5:7] / total_success if total_success > 0 else info_tensor[5:7]
            recall_info = recall_tensor.cpu().numpy() / total_final_stage if total_final_stage > 0 else \
                recall_tensor.cpu().numpy()
            ...
            self.logger.info(f"EP-{self.current_epoch} {'test/valid' if self.is_testing else 'train'} SUMMARY: " +
                             ...
                             f"({state_info[0]:.2f}, {state_info[1]:.2f}, "
                             f"{state_info[2]:.2f}, {state_info[3]:.2f}, {state_info[4]:.2f}) " +
                             f"t(perf, process)=({time_info[0]:.2f}s, {time_info[1]:.2f}s) " +
                             f"final_stage_cnt={cnt_tensor[2].item()} R@1={recall_info[0]} " +
                             f"R@3={recall_info[1]} R@5={recall_info[2]} R@10={recall_info[3]}")
            ...
            self.summary_writer.add_scalar("{0}/R@1_ordered".format(phase_tag), recall_info[0], global_step)
            self.summary_writer.add_scalar("{0}/R@3_ordered".format(phase_tag), recall_info[1], global_step)
            self.summary_writer.add_scalar("{0}/R@5_ordered".format(phase_tag), recall_info[2], global_step)
            self.summary_writer.add_scalar("{0}/R@10_ordered".format(phase_tag), recall_info[3], global_step)
            self.summary_writer.flush()


Student = UpdateActoreStudent
