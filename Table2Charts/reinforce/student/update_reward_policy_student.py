...
from data.update_reward_qvalues import QValue
from search.agent import ParallelAgents, SearchConfig
from search.agent.update_reward_policy_drill_down import UpdateRewardPolicyBeamDrillDownAgent
...
class Student:
    def __init__(self, config: StudentConfig, data_config: DataConfig, search_config: SearchConfig,
        ...
        self.apex = use_apex
        self.update_reward_negative_weight = getattr(config, "update_reward_negative_weight", 0.8)
        self.update_reward_positive_threshold = getattr(config, "update_reward_positive_threshold", 0.5)
        ...
        self.policy_epsilon_start = getattr(config, "policy_epsilon_start", 0.2)
        self.policy_epsilon_end = getattr(config, "policy_epsilon_end", 0.02)
        self.policy_epsilon_decay = getattr(config, "policy_epsilon_decay", 0.8)
        self.policy_explore_top_m = getattr(config, "policy_explore_top_m", 5)
    ...
    def add_table(self, tUID: str):
        ...
        self.agents.add(UpdateRewardPolicyBeamDrillDownAgent(tUID, self.data_config, self.special_tokens,
                                                             self.search_config))
    ...
    def reset(self, epoch: int, is_testing: bool):
        ...
        if self.is_testing:
            ...
            self.policy_epsilon = max(self.policy_epsilon_end,
                                      self.policy_epsilon_start * (self.policy_epsilon_decay ** max(epoch, 0)))
        self._sync_policy_config_()

    def _sync_policy_config_(self):
        self.search_config.policy_epsilon = self.policy_epsilon
        self.search_config.policy_explore_top_m = self.policy_explore_top_m
    ...
    def _dense_reward_loss_(self, output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        weights = self.update_reward_negative_weight + (1 - self.update_reward_negative_weight) * target
    ...
    def _feed_batch_nn_(self, mode: BatchMode, samples: List[QValue], noise_fn: Callable = None) -> np.ndarray:
        ...
        with torch.set_grad_enabled(mode is BatchMode.Train):
            if mode is BatchMode.Estimate:  # Return action value estimations
                ...
                y_true = (y_true[valid_b] >= self.update_reward_positive_threshold).astype(int)
