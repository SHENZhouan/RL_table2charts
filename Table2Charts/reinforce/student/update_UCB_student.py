...
from .update_UCB_config import StudentConfig
...
class Student:
    def __init__(self, config: StudentConfig, data_config: DataConfig, search_config: SearchConfig,
        ...
        self.ucb_exploration = getattr(config, "ucb_exploration", 0.5)
        self.ucb_eval_exploration = getattr(config, "ucb_eval_exploration", True)
    ...
    def _sync_policy_config_(self):
        self.search_config.ucb_exploration = self.ucb_exploration if (
            not self.is_testing or self.ucb_eval_exploration) else 0.0

    def add_table(self, tUID: str):
        self._sync_policy_config_()
        self.agents.add(UpdateUCBBeamDrillDownAgent(tUID, self.data_config, self.special_tokens, self.search_config))
