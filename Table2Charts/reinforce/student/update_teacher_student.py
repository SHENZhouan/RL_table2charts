...
from data import DataConfig, SpecialTokens, State, Sequence, get_template
from data.update_teacher_qvalues import QValue
from search.agent import ParallelAgents, SearchConfig
from search.agent.update_teacher_drill_down import UpdateTeacherBeamDrillDownAgent
...
class UpdateTeacherStudent:
    def __init__(self, config: StudentConfig, data_config: DataConfig, search_config: SearchConfig,
        ...
        self.apex = use_apex
        self.update_teacher_negative_weight = getattr(config, "update_teacher_negative_weight", 0.8)
        self.update_teacher_positive_threshold = getattr(config, "update_teacher_positive_threshold", 0.5)
        ...
        self.teacher_policy_epsilon_start = getattr(config, "teacher_policy_epsilon_start", 0.2)
        self.teacher_policy_epsilon_end = getattr(config, "teacher_policy_epsilon_end", 0.02)
        self.teacher_policy_epsilon_decay = getattr(config, "teacher_policy_epsilon_decay", 0.8)
        self.teacher_policy_explore_top_m = getattr(config, "teacher_policy_explore_top_m", 5)
        self.teacher_policy_epsilon = 0.0
    ...
    def add_table(self, tUID: str):
        ...
        self._sync_policy_config_()
        self.agents.add(UpdateTeacherBeamDrillDownAgent(tUID, self.data_config, self.special_tokens,
                                                        self.search_config))
    ...
    def reset(self, epoch: int, is_testing: bool):
        ...
        if self.is_testing:
            ...
            self.teacher_policy_epsilon = max(
                self.teacher_policy_epsilon_end,
                self.teacher_policy_epsilon_start * (self.teacher_policy_epsilon_decay ** max(epoch, 0))
            )
        self._sync_policy_config_()

    def _sync_policy_config_(self):
        self.search_config.teacher_policy_epsilon = self.teacher_policy_epsilon
        self.search_config.teacher_policy_explore_top_m = self.teacher_policy_explore_top_m
    ...
    def _dense_reward_loss_(self, output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        valid_mask = target >= 0
        if not valid_mask.any():
            return output.sum() * 0

        target = target.clamp(0, 1)
        pos_log_prob = output[:, :, 1]
        neg_log_prob = output[:, :, 0]
        loss = -(target * pos_log_prob + (1 - target) * neg_log_prob)

        weights = self.update_teacher_negative_weight + (1 - self.update_teacher_negative_weight) * target
        return (loss * weights)[valid_mask].sum()
    ...
    def _feed_batch_nn_(self, mode: BatchMode, samples: List[QValue], noise_fn: Callable = None) -> np.ndarray:
        ...
        with torch.set_grad_enabled(mode is BatchMode.Train):
            if mode is BatchMode.Estimate:  # Return action value estimations
                ...
                output = self.model(data["state"], data["actions"])  # Sync point!
                target = data["values"]
                loss = self._dense_reward_loss_(output, target)
                ...
                y_true = (y_true[valid_b] >= self.update_teacher_positive_threshold).astype(int)
    ...
    def train_supervised_batch(self, samples: List[QValue]):
        return self._feed_batch_nn_(BatchMode.Train, samples)

    def teacher_memory_samples(self) -> List[QValue]:
        return list(self.memory.memory)
