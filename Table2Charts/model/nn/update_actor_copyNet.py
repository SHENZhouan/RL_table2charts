...
def swap_field_cmd_scores(input_scores, num_cmd_tokens, source_mask):
    """
    Single-channel variant of swap_field_cmd_probs for actor logits.
    The critic path has a final class dimension of 2, but actor logits are
    shaped as batch * action_length and need their own scatter index.
    """
    batch_size, source_length = source_mask.size()
    ...
    return_scores = input_scores.new_zeros(input_scores.size())
    return_scores.scatter_(1, replace_index, input_scores)
    return return_scores


class UpdateActorCopyNetSeq2Seq(nn.Module):
    ...
    def __init__(self,
        ...
        self._actor_generation_layer = Linear(self.decoder_output_dim, self._command_token_size)
        self._actor_copying_layer = Linear(self.decoder_output_dim, self.decoder_output_dim)
    ...
    def forward(self, dqn_state, dqn_actions) -> torch.Tensor:
        ...
        if max_num_fields < 0:
            return {
                "critic_log_probs": dqn_actions["mask"].new_zeros((1, 2)),
                "actor_logits": dqn_actions["mask"].new_zeros((1,)),
            }
    ...
    def _get_actor_scores(self, state: Dict[str, torch.Tensor]) -> torch.Tensor:
        generation_scores = self._actor_generation_layer(state["decoder_hidden"])
        copy_projection = self._actor_copying_layer(state["encoder_outputs"])
        copy_projection = torch.tanh(copy_projection)
        copy_scores = copy_projection.bmm(state["decoder_hidden"].unsqueeze(-1)).squeeze(-1)
        return torch.cat((generation_scores, copy_scores), dim=-1)
    ...
    def _forward_estimation(self,
        ...
        # shape: (batch_size, dqn_state_length, command_token_size + source_length)
        actor_scores = torch.cat(actor_scores_lists, dim=1)
        actor_index = torch.sum(dqn_state_mask, 1, keepdim=True, dtype=torch.long) - 1
        actor_index = actor_index.expand(-1, actor_scores.size(-1)).unsqueeze(1)
        final_actor_scores = actor_scores.gather(1, actor_index).squeeze(1)
        final_actor_scores = swap_field_cmd_scores(final_actor_scores, self._command_token_size, source_mask)

        return {
            "critic_log_probs": final_step_log_probs,
            "actor_logits": final_actor_scores,
        }
    ...
    def get_decoder_modules(self):
        decoder_modules = [
            ...
            self._actor_generation_layer, self._actor_copying_layer
        ]
...
class UpdateActorCopyNet(UpdateActorCopyNetSeq2Seq):
