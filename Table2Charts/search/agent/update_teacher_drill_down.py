...
from data import DataConfig, State, DataTable, SpecialTokens, Result, get_template
from data.update_teacher_qvalues import QValue, TableQValues, determine_action_values
...
class UpdateTeacherBeamDrillDownAgent(Agent):
    def __init__(self, info: Union[str, dict], data_config: DataConfig, special_tokens: SpecialTokens,
        ...
        self.policy_rng = random.Random(getattr(search_config, "teacher_policy_seed", None))

    def _policy_param(self, name: str, default):
        return getattr(self.search_config, name, default)

    def _choose_continuation_idx(self, new_results: List[Result]) -> Optional[int]:
        if len(new_results) == 0:
            return None

        ranked = sorted(range(len(new_results)), key=lambda idx: new_results[idx].score, reverse=True)
        greedy_idx = ranked[0]
        epsilon = self._policy_param("teacher_policy_epsilon", 0.0)
        if epsilon <= 0.0 or self.policy_rng.random() >= epsilon:
            return greedy_idx

        explore_top_m = self._policy_param("teacher_policy_explore_top_m", 0)
        if explore_top_m and explore_top_m > 0:
            ranked = ranked[:min(explore_top_m, len(ranked))]
        return self.policy_rng.choice(ranked)
    ...
    def step(self) -> List[QValue]:
        ...
        for state in self.beam:
            ...
            action_values = determine_action_values(self.table.action_space, positive_actions, self.data_config)
    ...
    def update(self, chosen: List[QValue], predicted_values: Iterable) -> Optional[
        ...
        for state_actions, action_values in zip(chosen, predicted_values):
            ...
            best_idx = self._choose_continuation_idx(new_results)
