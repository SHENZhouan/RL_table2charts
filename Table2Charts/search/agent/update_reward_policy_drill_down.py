...
from data.update_reward_qvalues import QValue, TableQValues, determine_action_values
...
class UpdateRewardPolicyBeamDrillDownAgent(Agent):
    def __init__(self, info: Union[str, dict], data_config: DataConfig, special_tokens: SpecialTokens,
        ...
        self.policy_rng = random.Random(getattr(search_config, "policy_seed", None))
    ...
    def _choose_continuation_idx(self, new_results: List[Result]) -> Optional[int]:
        ...
        greedy_idx = ranked[0]
        epsilon = self._policy_param("policy_epsilon", 0.0)
        if epsilon <= 0.0 or self.policy_rng.random() >= epsilon:
            return greedy_idx

        explore_top_m = self._policy_param("policy_explore_top_m", 0)
