...
class UpdateUCBBeamDrillDownAgent(Agent):
    ...
    def _action_visit_key(self, state: State, action) -> tuple:
        return state, action

    def _ucb_score(self, state: State, action, critic_score: float) -> float:
        exploration = float(self._policy_param("ucb_exploration", 0.0))
        if exploration <= 0.0:
            return critic_score

        state_visits = self.state_visit_counts[state]
        action_visits = self.state_action_visit_counts[self._action_visit_key(state, action)]
        total = max(state_visits, 1)
        bonus = exploration * math.sqrt(math.log(total + 1.0) / (action_visits + 1.0))
        return critic_score + bonus

    def _choose_continuation_idx(self, state: State, actions: List, critic_scores: List[float]) -> Optional[int]:
        if len(actions) == 0:
            return None

        best_idx = None
        best_score = None
        for idx, (action, critic_score) in enumerate(zip(actions, critic_scores)):
            score = self._ucb_score(state, action, critic_score)
    ...
    def update(self, chosen: List[QValue], predicted_values: Iterable) -> Optional[dict]:
        ...
        for state_actions, action_values in zip(chosen, predicted_values):
            ...
            best_idx = self._choose_continuation_idx(state, valid_actions, critic_scores)
            ...
            if best_idx is not None:
                ...
                self.state_action_visit_counts[self._action_visit_key(state, best_action)] += 1
