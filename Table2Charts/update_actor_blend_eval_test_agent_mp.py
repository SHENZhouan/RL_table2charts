...
from model import DEFAULT_MODEL_SIZES, DEFAULT_MODEL_NAMES, get_cp_config
...
from search.agent.update_actor_drill_down import UpdateActorBeamDrillDownAgent
from threading import Thread
from time import perf_counter
from tqdm import tqdm
from typing import List
from update_actor_helper import load_update_actor_checkpoint
from util import to_device, time_str
...
def parse_args():
    parser = argparse.ArgumentParser(description="Concurrent Blend-Eval Test Search Agents")
    ...
    parser.add_argument("--critic_score_weight", default=0.5, type=float,
                        help="Blend weight for critic scores. 1.0 uses critic only; 0.0 uses actor only.")
...
def create_model(device, args):
    ...
    if args.model_name == "cp":
        ...
        model = UpdateActorCopyNet(cp_config)
        logger.info("UpdateActorCopyNetConfig: {}".format(vars(cp_config)))
    else:
        raise NotImplementedError(f"{args.model_name} not yet implemented.")

    if args.model_file:
        load_update_actor_checkpoint(os.path.join(args.model_dir_path, args.model_file), model, device=device)
    model.to(device)
    model.eval()
    return model
...
def create_agent(tUID: str, special_tokens: SpecialTokens, args) -> Agent:
    ...
    if args.search_algo == "drill_down":
        return UpdateActorBeamDrillDownAgent(tUID, data_config, special_tokens, search_config)
    else:
        raise ValueError(f"No {args.search_algo} search agent not implemented!")


def feed_batch_nn(samples: List[QValue], model: torch.nn.Module, device: str, config: DataConfig,
                  critic_score_weight: float) -> np.ndarray:
    if len(samples) == 0:
        return np.empty([0])
    with torch.no_grad():
        ...
        actor_probs = output["actor_logits"].masked_fill(torch.logical_not(valid_mask), -1e9).softmax(dim=-1)
        scores = critic_score_weight * critic_scores + (1.0 - critic_score_weight) * actor_probs
        return scores.masked_fill(torch.logical_not(valid_mask), 0.0).cpu()
...
def process_parallel(index, device_count, special_tokens: SpecialTokens,
    ...
    while True:
        ...
        futures = agents.step([
            lambda samples: feed_batch_nn(samples, dqn, device, data_config, args.critic_score_weight)
        ])
...
def summary(info_queue: mp.Queue, args):
    ...
    with open(log_file_path, "w") as log_file:
        ...
        log_file.flush()
        logger.info("Evaluation scoring mode = blend (critic_score_weight=%.4f)", args.critic_score_weight)
...
def test(args):
    ...
    logger.info("Evaluation scoring mode = blend (critic_score_weight=%.4f)", args.critic_score_weight)
    logger.info("CUDA visibility: is_available=%s device_count=%s CUDA_VISIBLE_DEVICES=%s",
                torch.cuda.is_available(), torch.cuda.device_count(), os.environ.get("CUDA_VISIBLE_DEVICES"))
    ...
    if device_count <= 0:
        raise RuntimeError("No CUDA devices visible to evaluation process. "
    ...
    if args.nprocs > 1 and device_count:
