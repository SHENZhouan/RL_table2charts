...
def parse_args():
    parser = argparse.ArgumentParser(description="Concurrent Critic-Only Test Search Agents")
...
def feed_batch_nn(samples: List[QValue], model: torch.nn.Module, device: str, config: DataConfig) -> np.ndarray:
    ...
    with torch.no_grad():
        ...
        return critic_scores.masked_fill(torch.logical_not(valid_mask), 0.0).cpu()
...
def summary(info_queue: mp.Queue, args):
    ...
    with open(log_file_path, "w") as log_file:
        log_file.write("[meta]\n")
        log_file.write(json.dumps({"scoring_mode": "critic_only"}, sort_keys=True, indent=4))
        log_file.write("\n\n")
        while True:
            info = info_queue.get()
            if info is not None:
        ...
        log_file.flush()
        logger.info("Evaluation scoring mode = critic_only")
