...
from model.nn.update_actor_new_copyNet import UpdateActorNewCopyNet
...
def load_update_actor_new_checkpoint(file_path: str, module: Module, device="cpu"):
    states = load_states(file_path)
    incompatible = module.load_state_dict(states["module_state"], strict=False)
    missing = getattr(incompatible, "missing_keys", [])
    unexpected = getattr(incompatible, "unexpected_keys", [])
    logger = logging.getLogger(f"load_update_actor_new_checkpoint({_dist_rank_or_default()})")
    if missing:
        logger.info("Missing keys while loading checkpoint into update_actor_new model: %s", missing)
    if unexpected:
        logger.info("Unexpected keys while loading checkpoint into update_actor_new model: %s", unexpected)
    module.to(device)
...
def create_model(args):
    ...
    if args.model_name == "cp":
        ...
        model = UpdateActorNewCopyNet(model_config)
        if rank == 0:
            logger.info("UpdateActorNewCopyNetConfig: {}".format(vars(model_config)))
    else:
        raise NotImplementedError(f"{args.model_name} not yet implemented.")

    experiment_name = datetime.now(timezone(timedelta(hours=8))).strftime("%Y%m%d%H%M%S-update_actor_new-") + \
...
def prepare_model(model: Module, device, args):
    ...
    if args.pre_model_file:
        ...
        load_update_actor_new_checkpoint(args.pre_model_file, model, device)
    else:
    ...
    if args.fresh_decoder:
        if isinstance(model, UpdateActorNewCopyNet):
