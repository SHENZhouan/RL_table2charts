...
from update_actore_helper import load_update_actore_checkpoint
...
def create_model(device, args):
    ...
    if args.model_name == "cp":
        ...
        model = UpdateActoreCopyNet(cp_config)
        logger.info("UpdateActoreCopyNetConfig: {}".format(vars(cp_config)))
    else:
        raise NotImplementedError(f"{args.model_name} not yet implemented.")

    if args.model_file:
        load_update_actore_checkpoint(os.path.join(args.model_dir_path, args.model_file), model, device=device)
    model.to(device)
    model.eval()
    return model
...
def create_agent(tUID: str, special_tokens: SpecialTokens, args) -> Agent:
    ...
    if args.search_algo == "drill_down":
        return UpdateActoreBeamDrillDownAgent(tUID, data_config, special_tokens, search_config)
    else:
