...
from search.agent import DEFAULT_SEARCH_LIMITS, get_search_config, SearchConfig, Agent, ParallelAgents
from search.agent.update_UCB_drill_down import UpdateUCBBeamDrillDownAgent
...
def parse_args():
    parser = argparse.ArgumentParser(description="Concurrent Test Search Agents for UCB search")
    ...
    parser.add_argument('--ucb_exploration', default=0.5, type=float,
                        help='exploration coefficient for UCB over critic scores during test search')
...
def construct_search_config(args, data_config) -> SearchConfig:
    ...
    search_config.ucb_exploration = args.ucb_exploration
...
def create_agent(tUID: str, special_tokens: SpecialTokens, args) -> Agent:
    ...
    if args.search_algo == "drill_down":
        return UpdateUCBBeamDrillDownAgent(tUID, data_config, special_tokens, search_config)
...
def process_parallel(index, device_count, special_tokens: SpecialTokens,
    ...
    while True:
        ...
        futures = agents.step([lambda samples: feed_batch_nn(samples, dqn, device, data_config)])
