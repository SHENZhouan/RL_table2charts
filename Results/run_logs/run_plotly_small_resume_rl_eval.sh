...
STATUS_FILE="${LOG_DIR}/plotly_small_resume_rl_eval_${RUN_ID}.status"
MARKER_FILE="${LOG_DIR}/plotly_small_resume_rl_eval_${RUN_ID}.marker"
...
SFT_CKPT="${SFT_CKPT:?SFT_CKPT must point to a completed SFT states_ep0.pt}"
SFT_DIR="$(dirname "${SFT_CKPT}")"

mkdir -p "${LOG_DIR}"
touch "${LOG_FILE}" "${STATUS_FILE}" "${MARKER_FILE}"
ln -sfn "${LOG_FILE}" "${LOG_DIR}/plotly_small_resume_rl_eval.latest.log"
ln -sfn "${STATUS_FILE}" "${LOG_DIR}/plotly_small_resume_rl_eval.latest.status"
...
echo "[RL] resume from SFT, full Plotly corpus, 1 epoch, local sharded queue"
