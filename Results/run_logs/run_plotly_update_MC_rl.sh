...
LOG_FILE="${LOG_DIR}/plotly_update_MC_rl_${RUN_ID}.log"
STATUS_FILE="${LOG_DIR}/plotly_update_MC_rl_${RUN_ID}.status"
RESULT_FILE="${LOG_DIR}/plotly_update_MC_rl_${RUN_ID}.results"
MARKER_FILE="${LOG_DIR}/plotly_update_MC_rl_${RUN_ID}.marker"
...
MC_TOP_K="${MC_TOP_K:-3}"
MC_ROLLOUT_DEPTH="${MC_ROLLOUT_DEPTH:-2}"
MC_NUM_ROLLOUTS="${MC_NUM_ROLLOUTS:-2}"
MC_DISCOUNT="${MC_DISCOUNT:-0.9}"
MC_ROLLOUT_WEIGHT="${MC_ROLLOUT_WEIGHT:-0.5}"
EPOCHS="${EPOCHS:-1}"
...
ln -sfn "${LOG_FILE}" "${LOG_DIR}/plotly_update_MC_rl.latest.log"
ln -sfn "${STATUS_FILE}" "${LOG_DIR}/plotly_update_MC_rl.latest.status"
ln -sfn "${RESULT_FILE}" "${LOG_DIR}/plotly_update_MC_rl.latest.results"
...
on_error() {
  ...
  append_results ""
  append_results "### Plotly Update MC failed"
  append_results ""
...
write_status "mc_discount=${MC_DISCOUNT}"
...
write_result "mc_top_k=${MC_TOP_K}"
write_result "mc_rollout_depth=${MC_ROLLOUT_DEPTH}"
write_result "mc_num_rollouts=${MC_NUM_ROLLOUTS}"
write_result "mc_discount=${MC_DISCOUNT}"
write_result "mc_rollout_weight=${MC_ROLLOUT_WEIGHT}"
...
write_result "train_table_offset=${TRAIN_TABLE_OFFSET}"
write_result "valid_table_offset=${VALID_TABLE_OFFSET}"
write_result "train_table_stride=${TRAIN_TABLE_STRIDE}"
write_result "valid_table_stride=${VALID_TABLE_STRIDE}"

append_results ""
append_results "## Plotly Update MC ${RUN_ID}"
append_results ""
append_results "- started_utc: $(date -u +%Y-%m-%dT%H:%M:%SZ)"
append_results "- gpu_ids: ${GPU_IDS}"
append_results "- sft_ckpt: ${SFT_CKPT}"
append_results "- mc_top_k: ${MC_TOP_K}"
append_results "- mc_rollout_depth: ${MC_ROLLOUT_DEPTH}"
append_results "- mc_num_rollouts: ${MC_NUM_ROLLOUTS}"
append_results "- mc_discount: ${MC_DISCOUNT}"
append_results "- mc_rollout_weight: ${MC_ROLLOUT_WEIGHT}"
...
append_results "- train_table_offset: ${TRAIN_TABLE_OFFSET}"
append_results "- valid_table_offset: ${VALID_TABLE_OFFSET}"
append_results "- train_table_stride: ${TRAIN_TABLE_STRIDE}"
append_results "- valid_table_stride: ${VALID_TABLE_STRIDE}"
append_results "- log_file: ${LOG_FILE}"

test -f "${SFT_CKPT}"
cd "${CODE_DIR}"

write_status "status=update_MC_rl_running"
echo "[RL:update_MC] resume from SFT checkpoint, top-k action + shallow MC rollout over critic scores"
CUDA_VISIBLE_DEVICES="${GPU_IDS}" "${PY}" -m torch.distributed.launch \
  --nproc_per_node="${RL_NPROCS}" \
  --master_port="${MASTER_PORT}" \
  --module reinforce.update_MC_learn_dist \
...
RL_DIR="$(find "${ROOT}/Results/Models" -maxdepth 1 -type d -newer "${MARKER_FILE}" -name '*-MC-RL' -printf '%T@ %p\n' | sort -nr | head -1 | cut -d' ' -f2-)"
