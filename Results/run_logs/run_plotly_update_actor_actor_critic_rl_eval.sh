...
LOG_FILE="${LOG_DIR}/plotly_update_actor_actor_critic_rl_eval_${RUN_ID}.log"
STATUS_FILE="${LOG_DIR}/plotly_update_actor_actor_critic_rl_eval_${RUN_ID}.status"
RESULT_FILE="${LOG_DIR}/plotly_update_actor_actor_critic_rl_eval_${RUN_ID}.results"
MARKER_FILE="${LOG_DIR}/plotly_update_actor_actor_critic_rl_eval_${RUN_ID}.marker"
...
ln -sfn "${LOG_FILE}" "${LOG_DIR}/plotly_update_actor_actor_critic_rl_eval.latest.log"
ln -sfn "${STATUS_FILE}" "${LOG_DIR}/plotly_update_actor_actor_critic_rl_eval.latest.status"
ln -sfn "${RESULT_FILE}" "${LOG_DIR}/plotly_update_actor_actor_critic_rl_eval.latest.results"
...
on_error() {
  ...
  append_results ""
  append_results "### Plotly Update Actor Actor-Critic failed"
  append_results ""
...
append_results ""
append_results "## Plotly Update Actor Actor-Critic ${RUN_ID}"
append_results ""
...
write_status "status=update_actor_actor_critic_rl_running"
echo "[RL:update_actor_actor_critic] resume from SFT checkpoint, actor+critic blend search"
CUDA_VISIBLE_DEVICES="${GPU_IDS}" "${PY}" -m torch.distributed.launch \
  --nproc_per_node="${RL_NPROCS}" \
  --master_port="${MASTER_PORT}" \
  --module reinforce.update_actor_learn_dist \
...
RL_DIR="$(find "${ROOT}/Results/Models" -maxdepth 1 -type d -newer "${MARKER_FILE}" -name '*update_actor-*allCharts-actor-critic-RL' -printf '%T@ %p\n' | sort -nr | head -1 | cut -d' ' -f2-)"
...
write_status "status=update_actor_actor_eval_running"
echo "[EVAL:update_actor_actor] Plotly test split, actor policy scores"
...
EVAL_DIR="${RL_DIR}/evaluations/test-update_actor-actor-critic-plotly-small-${RUN_ID}"
write_result "evaluation_log_dir=${EVAL_DIR}"

append_results ""
append_results "### Update Actor Actor-Critic RL + Actor Evaluation"
append_results ""
