...
LOG_FILE="${ROOT}/Results/run_logs/plotly_update_actor_critic_only_eval_${RUN_ID}.log"
STATUS_FILE="${ROOT}/Results/run_logs/plotly_update_actor_critic_only_eval_${RUN_ID}.status"
RESULT_FILE="${ROOT}/Results/run_logs/plotly_update_actor_critic_only_eval_${RUN_ID}.results"
RESULTS_MD="${ROOT}/results.md"
EVAL_DIR="${MODEL_DIR}/evaluations/test-update_actor-critic-only-plotly-small-${RUN_ID}"
...
write_status "scoring=critic_only"
write_status "status=eval_running"

cd "${CODE_DIR}"
CUDA_VISIBLE_DEVICES="${GPU_IDS}" "${PY}" update_actor_critic_only_test_agent_mp.py \
  -m "${MODEL_DIR}" \
  -f states_ep0.pt \
  --model_name cp \
  --model_size small \
  --features all-fast \
  --log_save_path "evaluations/test-update_actor-critic-only-plotly-small-${RUN_ID}" \
...
{
  printf '\n'
  printf '### Update Actor critic-only eval %s\n' "${RUN_ID}"
  printf '\n'
  printf -- '- finished_utc: %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf -- '- model_dir: %s\n' "${MODEL_DIR}"
  printf -- '- model_ckpt: %s\n' "${MODEL_DIR}/states_ep0.pt"
  ...
  printf -- '- log_file: %s\n' "${LOG_FILE}"
  printf -- '- scoring: critic_only\n'
  printf '\n```text\n'
  grep -E "Evaluation scoring mode = critic_only|SUMMARY:|Complete recall info|Complete R@|R@1=|R@3=|R@5=|R@10=" "${LOG_FILE}" | tail -80 || true
  printf '```\n'
} >> "${RESULTS_MD}"
